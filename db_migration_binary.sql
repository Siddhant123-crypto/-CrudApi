-- 1. Insert all users who have role = 'CUSTOMER' into customer table
INSERT INTO customer (customer_id, user_id)
SELECT UUID_TO_BIN(UUID()), user_id FROM users WHERE role = 'CUSTOMER';

-- 3. Move farmer_profile data into users
-- Create a temporary mapping table to hold new user_id for each farmer email
...
-- 4. Insert into users from farmer_profile using the generated UUIDs
INSERT INTO users (
...
SELECT 
    t.new_user_id, f.name, f.email, f.mobile, f.password, f.state, f.village, f.address, f.postal_code, f.profile_photo,
...
-- 7. Drop duplicate columns from farmer_profile
