-- Update duplicate mobiles to be unique internally
UPDATE farmer_profile t1
JOIN (
    SELECT id, mobile, ROW_NUMBER() OVER (PARTITION BY mobile ORDER BY id) as rn
    FROM farmer_profile
) t2 ON t1.id = t2.id
SET t1.mobile = CONCAT(SUBSTRING(t1.mobile, 1, 6), SUBSTRING(UUID(), 1, 4))
WHERE t2.rn > 1;
