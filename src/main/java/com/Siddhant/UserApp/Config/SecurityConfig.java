...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {http.csrf(csrf -> csrf.disable()).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            .authorizeHttpRequests(auth -> auth
                    .requestMatchers(
                                "/user/register", "/user/login", "/user/google-login", "/newuser/google-login", "/newuser/login", "/farmer/save", "/farmer/login", "/farmer/state/**", "/farmer/nearby", "/user/image/**", "/user/getPhoto/**", "/product/image/**", "/product/video/**", "/auth/**"
