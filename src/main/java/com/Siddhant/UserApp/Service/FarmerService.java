...
public abstract class FarmerService {
    ...
    public FarmerResponse saveFarmer(FarmerRequest request, MultipartFile photo) {
        ...
        if (farmerProfileRepository.existsByEmail(request.getEmail())) {
        ...
        if (farmerProfileRepository.existsByMobile(request.getMobile())) {
    ...
    public abstract List<FarmerResponse> getFarmersByState(String state);

    public abstract List<FarmerResponse> getNearbyFarmers(String state, String village);
}